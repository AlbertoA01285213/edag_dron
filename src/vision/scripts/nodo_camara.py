...
class GzToRosBridge(Node):
    def __init__(self):
        ...
        self.declare_parameter('gz_topic', '/camera_main')
        gz_topic_name = self.get_parameter('gz_topic').get_parameter_value().string_value

        self.publisher_ = self.create_publisher(Image, 'camara_dron/image_raw', 10)
        self.bridge = CvBridge()

        self.gz_node = GzNode()
        self.gz_node.subscribe(GzImage, gz_topic_name, self.gz_callback)
